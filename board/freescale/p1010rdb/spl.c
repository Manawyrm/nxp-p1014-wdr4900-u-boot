...
ulong kitten_memtest(vu_long *buf, ulong start_addr, ulong end_addr, ulong seed)
{
	...
	printf("KittenMemtest PRBS15, writing test bseudo random test sequence\n");
	...
	for (addr = buf; addr < end; addr++)
	{
		...
		*addr = prbslong(&prbs_state);
		...
		if (((ulong)addr & 0xFFFFFul) == 0)
		{
			printf("\rWriting 0x%08lx", (ulong)addr);
		}
	}
...
static int do_mem_mtest()
{
	ulong start, end;
	vu_long scratch_space;
	vu_long *buf, *dummy = &scratch_space;
	ulong iteration_limit = 10;
	ulong count = 0;
	ulong errs = 0;	/* number of errors, or -1 if interrupted */
	ulong pattern = 0;
	int iteration;

	//printf("dcache_disable()\n");
	//dcache_disable();

	//printf("invalidate_dcache()\n");
	//invalidate_dcache();
	...
	for (iteration = 0;	!iteration_limit || iteration < iteration_limit; iteration++) {
		...
		errs = kitten_memtest(buf, start, end, iteration);
